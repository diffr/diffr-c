...
static void instructions_foreach(gpointer data, gpointer user_data) {
  ...
  if (i->type == COPY) {
    long count = i->data.range->start - 1;
    long stop = i->data.range->end - 1;
...
GSList* parse_instructions(char *filename) {
  ...
  while (g_io_channel_read_line_string(
    ...
    if (*(newstr->str) == '>') {
      ...
      copy_range range = {.start = (long) atoi(start), .end = (long) atoi(end)};
      i = create_copy_instruction(&range);
      g_free(start);
      g_free(end);
    }
